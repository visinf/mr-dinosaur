...
class CombinedModel(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        batch_size = batch["batch_size"]
        outputs = self(batch)
        quantities_to_log = self._compute_metrics(outputs, self.evaluation_metrics, phase="val")

        self.log_dict(
            quantities_to_log, on_step=False, on_epoch=True, prog_bar=True, batch_size=batch_size
        )

        self._log_visualizations(outputs, phase="val",batch_idx=batch_idx)
    ...
    def test_step(self, batch, batch_idx):
        outputs = self(batch)
        del outputs["model"]
        self._log_visualizations(outputs, phase="test",batch_idx=batch_idx)
        return outputs
    ...
    def _compute_metrics(self, outputs, metric_fns, phase="train"):
        ...
        return metrics
    def _log_visualizations(self, outputs, phase="train",batch_idx=None):
        ...
        for path, vis in visualization_iterator:
            try:
                str_path = ".".join(path)
                global_step = self.trainer.global_step if phase == "train" else batch_idx
