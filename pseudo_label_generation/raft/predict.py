    ...
    with torch.no_grad():
        ...
        for index, imfile1 in enumerate(images_):
            if args.reverse:
                ...
                svfile = imfile1
            original_size = (image1.size(2), image1.size(3)) 
            padder = InputPadder(image1.shape)
            image1, image2 = padder.pad(image1, image2)

            flow_low, flow_up = model(image1, image2, iters=20, test_mode=True)

            flow_up_resized = torch.nn.functional.interpolate(flow_up, size=original_size, mode='bilinear', align_corners=False)

            flopath = os.path.join(floout, os.path.basename(svfile))
            rawflopath = os.path.join(rawfloout, os.path.basename(svfile))

            flo = flow_up_resized[0].permute(1, 2, 0).cpu().numpy()
