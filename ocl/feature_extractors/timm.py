...
class TimmFeatureExtractor(ImageFeatureExtractor):
    ...
    def __init__(
        ...
        if self.is_vit:
            ...
            if len(self.feature_levels) > 0 or len(self.aux_features) > 0:
                ...
                if len(self.feature_levels) > 0:
                    ...
                    new_blocks = model.blocks[:max_block]
                    del model.blocks
        ...
        if self.n_blocks_to_unfreeze > 0:
            if not self.is_vit:
                raise NotImplementedError("`unfreeze_n_blocks` option only implemented for ViT models")
...
class DINOv2FeatureExtractor(ImageFeatureExtractor):
    """Feature extractor implementation for DINOv2 models."""
    def __init__(
        self,
        model_name: str="",
        feature_level: Optional[Union[int, str, List[Union[int, str]]]] = None,
        ...
        if model_name.startswith("vits"):
            ...
            feature_dim = 768
        print("dinov2 model is loaded")
        if hasattr(model, "head"):
            print("delete head !")
            del model.head  
        if hasattr(model, "fc_norm"):
            print("delete fc_norm !")
            del model.fc_norm  
        def feature_level_to_list(feature_level):
            if feature_level is None:
                return []  
    ...
    def forward_images(self, images: torch.Tensor):
        ...
        if self.freeze and self.n_blocks_to_unfreeze == 0:
            with torch.no_grad():
                features = self.model.get_intermediate_layers(images, self.feature_levels, return_class_token=False )
        else:
            features = self.model.get_intermediate_layers(images, self.feature_levels, return_class_token=False)
        if self.patch_size is None:
            positions = transformer_compute_positions(features[0])
        else:
            positions = transformer_compute_positions(features[0],self.patch_size)
        features = torch.cat(features, dim=-1)
