        ...
        if image.dim() == 4:  # Only support image data at the moment.
            ...
            masks_on_image = torch.stack(
                [
                    ...
                    for img, mask in zip(input_image.to("cpu"), mask.to("cpu"))
                ]
            )
...
class SegmentationOut(Segmentation):
    ...
    def __init__(
        ...
        Args:
            ...
            denormalization: Function to map from normalized inputs to unnormalized values
        """
        super().__init__(n_instances=n_instances, denormalization=denormalization)
        self.num = num
    ...
    def _mask_to_boundary(mask: torch.Tensor, kernel_size: int = 3) -> torch.Tensor:
        """
        Compute a 1-pixel boundary of a single binary mask.
        ...
        Returns:
            Bool tensor (H, W) where boundary pixels are True
        """
    ...
    def __call__(
        ...
        assert image.dim() == 4 and mask.dim() == 4

        slot_num= mask.shape[1]
        ignore_channel = torch.arange(0, slot_num, slot_num // self.windows)
        ...
        keep = torch.ones(slot_num, dtype=torch.bool, device=masks_one_hot.device)
        keep[ignore_channel] = False
        masks_one_hot = masks_one_hot[:, keep, :, :]
        channel_nonzero = masks_one_hot.sum(dim=(0, 2, 3)) > 0  # Bool tensor, shape: (N,)
        masks_one_hot = masks_one_hot[:, channel_nonzero, :, :]  # (B, K, H, W)  
              
        viz_list = []
        cmap = self._get_cmap(masks_one_hot.shape[1])
        for img_i, msk_i in zip(input_image.to("cpu"), masks_one_hot.to("cpu")):
            overlay = draw_segmentation_masks(
                (255 * img_i).to(torch.uint8), msk_i, alpha=0.75, colors=cmap
            )
            viz_list.append(overlay)

        masks_on_image = torch.stack(viz_list)  
        if self.output_path is not None:
            for img in masks_on_image:  
                filename = os.path.join(self.output_path, f"{self.num}.png")
